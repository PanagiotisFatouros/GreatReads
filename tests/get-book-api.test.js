...
test('Valid Book', async ({ page, baseURL }) => {
	...
	const response = await page.goto(`localhost:3000/api/read/books/${validBookId}/${testUser.id}`);
	await expect(response?.ok()).toBeTruthy()
});

test('Invalid Book - Book Landing Page', async({ page, baseURL }) => {
	const invalidBookId = "nosuchbook12"
	const response = await page.goto(`localhost:3000/api/read/books/${invalidBookId}}/${testUser.id}`);
