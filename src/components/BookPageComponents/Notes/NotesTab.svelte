...
<div class="flex flex-col justify-start w-full">
    ...
    {#if $isOverlayOpen && selectedCollection == null}
        <form on:submit={createNewCollection} class=" bg-white fixed z-10 self-center w-96 flex flex-col p-3 rounded-2xl">
            <h2 class=" text-secondary">New Notes Collection</h2>
            
            <input bind:value={newCollectionTitle} type="text" placeholder="Title..." class=" standard_input my-2">
            ...
            <div class="flex justify-between text-body2 space-x-2 mt-1 w-full items-center">
                ...
                <div class="">
                    <button on:click={() => isOverlayOpen.set(false)} class=" btn bg-primary-1 rounded-full px-2"><p>Cancel</p></button>
                    <button type="submit" class="btn bg-accent text-white rounded-full px-2"><p>Create</p></button>
                </div>
    ...
    {:else}
        ...
        <div class=" mt-3 flex">
            ...
            <button on:click={() => isOverlayOpen.set(true)} class="btn rounded-full w-6 h-6 text-white bg-accent self-center"><p class=" text-xl leading-none">+</p></button>
        </div>
