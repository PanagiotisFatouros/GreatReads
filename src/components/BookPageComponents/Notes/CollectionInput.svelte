...
>
	...
	<div class="flex justify-between text-body2 space-x-2 mt-1 w-full items-center">
		...
		<div class=" mt-1">
			<button on:click={handleCancel} class=" btn bg-primary-1 rounded-full px-3 py-0.5"
				><p>Cancel</p></button
			>
			{#if collection == undefined}
				<button
					on:click={createNewCollection}
					class="btn bg-accent text-white rounded-full px-3 py-0.5"><p>Create</p></button
				>
			{:else}
				<button
					on:click={updateCollection}
					class="btn bg-accent text-white rounded-full px-3 py-0.5"><p>Update</p></button
				>
			{/if}
		</div>
	</div>
