...
<div id="card" class="bg-primary-1">
	...
	{:else}
		<p class="font-body text-primary-3">
			Favourite Author: <span
				on:click={() => goto(`${baseURL}/search?Authors=${user.favAuthor}`)}
				class="text-primary-2 cursor-pointer underline hover:text-accent">{user.favAuthor}</span
			>
		</p>
	{/if}
	...
	{:else}
		<p class="font-body text-primary-3">
			Favourite Genre: <span
				on:click={() => goto(`${baseURL}/search?Genres=${user.favGenre}`)}
				class="text-primary-2 cursor-pointer underline hover:text-accent">{user.favGenre}</span
			>
		</p>
	{/if}
</div>
