<script lang="ts">
	...
	$: {
		if (
			data.searchType == searchTypes.books ||
...
{#if isBookSearch}
	...
	<div class="flex flex-col justify-start w-full">
		...
		{#if sortOn && $isOverlayOpen && booksShown != undefined}
			<div class="z-10 fixed self-center">
				<SortPanel bind:show={sortOn} {books} bind:booksShown bind:sortOption />
			</div>
		{/if}
	</div>
	...
	<div class="mx-6 flex flex-row flex-wrap justify-start items-center">
		{#if booksShown.length > 0}
			{#each booksShown as book}
				<BookCard on:click={() => goto(`/books/${book.id}`)} {book} {bookshelves} />
			{/each}
		{:else}
