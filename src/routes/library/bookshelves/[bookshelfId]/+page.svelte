...
<div class="flex flex-col justify-start w-full">
	...
	{#if sortOn && bookshelf.books != undefined}
		<div class="z-10 fixed self-center">
			<SortPanel bind:show={sortOn} books={bookshelf.books} bind:booksShown bind:sortOption />
		</div>
	{/if}
	{#if isDeleting}
		<Confirmation
			title={'Delete Bookshelf'}
			description={'Are you sure you want to delete this bookshelf? <br/>This can not be undone.'}
			on:cancel={() => (isDeleting = false)}
			on:confirm={deleteBookshelf}
		/>
	{/if}
	{#if isEditing}
		<AddBookshelfPanel bind:show={isEditing} bind:bookshelf />
	{/if}
</div>
	...
	{#if booksShown != undefined && booksShown.length > 0}
		{#each booksShown as book}
			<BookCard
				...
				on:maybeRemoved={(event) => handleMaybeRemoved(event, book.id)}
				{book}
